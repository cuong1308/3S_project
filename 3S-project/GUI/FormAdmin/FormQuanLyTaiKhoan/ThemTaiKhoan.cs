...
namespace _3S_project.GUI.FormAdmin.FormQuanLyTaiKhoan
{
    public partial class ThemTaiKhoan : Form
    {
        public ThemTaiKhoan()
        {
            ...
            lstGV.Insert(0, new Model.GiangVien() { TenGiangVien = "Không"});
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(tenDangNhap) || string.IsNullOrWhiteSpace(email) || string.IsNullOrWhiteSpace(pass) || string.IsNullOrWhiteSpace(passconf))
            {
            ...
            if(giangVien.TenGiangVien != "Không")
            {
                if(pass == passconf)
                {
                    ...
                    if (dtTK.Them(tenDangNhap,giangVien.MaGiangVien.ToString(),3,pass,email))
                    {
            ...
            else
            {
                if (pass == passconf)
                {
                    if (dtTK.Them(tenDangNhap, "0", 2, pass, email))
                    {
                        MessageBox.Show("Thêm tài khoản thành công!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        this.DialogResult = DialogResult.OK;
                        this.Close();
